...
def main():
    ...
    msg = (f'({context.current_model.name}) has ran' 
           f'with {context.current_model.status}, '
           f'and has {number_of_rows} rows,'
           f'notifying <{context.current_model.meta["owner"]}>'
    )
    send_slack_message(msg, CHANNEL_ID, SLACK_TOKEN)
    
def send_slack_message(
    message_text: str, channel_id: str, slack_token: str
    ):
    client = WebClient(token=slack_token)
